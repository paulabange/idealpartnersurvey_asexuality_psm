...
bal.plot(nnm_25, var.name = "country", which = "both", grid = T) # recht unübersichtlich
...
# remove country variable for love plot
nnm__25_without_country = nnm_25
nnm__25without_country$X[c("country")] <- NULL 
...
bal.plot(nnm_10, var.name = "country", which = "both", grid = T) # recht unübersichtlich
...
```{r}
# MatchIt does not allow missing values
data_main_nomiss_self <- data_main %>% select(language, age, sex, country, relationship_status, relationship_length, sexual_orientation,
...
dim(nnm_self_data) # 772 observations and 16 variables
```
No asexual unmatched. 
...
bal.plot(nnm_self, var.name = "country", which = "both", grid = T) # recht unübersichtlich
...
# histogram sexual experience by sexual orientation
ggplot(nnm_self_data, aes(x = self_sexually_experienced, fill = sexual_orientation)) + 
  geom_histogram(position = "dodge")
```
Sexual experience:mean asexual = 1.05; mean hetero = 2.13
...
```{r}
# MatchIt does not allow missing values
data_main_nomiss_rel <- data_main %>% select(language, age, sex, country, relationship_status, relationship_length,
                                             sexual_orientation,
                                             interest_hookups,
                                             interest_nonsexrel,
                                             interest_monrel,
...
nrow(nnm_rel_data) # 652 observations
table(nnm_rel_data$subclass) # data includes only pairs of two observations
dim(nnm_rel_data) # 652 observations and 16 variables
```
No asexual unmatched. 
...
bal.plot(nnm_rel, var.name = "country", which = "both", grid = T) # recht unübersichtlich
...
# for preferred relationship options
