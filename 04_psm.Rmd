...
```
Based on SMDs, data is not balanced for 5 covariates (langauge_english, language_german, age, country_germany, country_United States of America) . For continuous variables variance ratios are close to 1.
...
nnm_10_discard <- matchit(sexual_orientation ~ language + age + country + relationship_status + relationship_length, method = "nearest", caliper = 0.1, discard = "both", data = data_main_nomiss)

nrow(nnm_10_discard$match.matrix)
nnm_10_data <- match.data(nnm_25)
nrow(nnm$match.matrix)
?matchit
nrow(nnm_10_data) # 778 observations
table(nnm_10_data$subclass) # data includes only pairs of two observations
dim(nnm_10_data) # 778 observations and 16 variables
```
