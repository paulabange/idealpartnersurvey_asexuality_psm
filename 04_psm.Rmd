...
round(bal.tab(om, m.threshold = 0.1, un = T, binary = "std"),2) #smd
...
print(summary_nnm)
round(head(summary_nnm$sum.all),2)
round(summary_nnm$sum.all,2)
round(tail(summary_nnm$sum.all),2)
round(tail(summary_nnm$sum.matched),2)
round(head(summary_nnm$sum.matched),2)
round(summary_nnm$sum.matched, 2)


bal.tab(nnm, m.threshold = 0.1, un = T, binary = "std") 
bal.tab(nnm, m.threshold = 0.1, un = T) #smd
bal.tab(nnm, v.threshold = 2, un = T) #vr (can only be calculated for continuous variables)

bal.tab(nnm, m.threshold = 0.1, un = T, binary = "std", s.d.denom = "pooled") 
bal.tab(nnm, m.threshold = 0.1, un = T, binary = "std", s.d.denom = "treated")
?bal.tab
```
...
love.plot(sexual_orientation ~ language + age + 
          ...
          thresholds = c(m = .1),
          s.d.denom = "pooled",
          sample.names = c("Unmatched", "Optimal Matching", "Nearest-Neighbor-Matching", "NNM with caliper = 0.25", "NNM with caliper = 0.10"),
          var.names = v) 
