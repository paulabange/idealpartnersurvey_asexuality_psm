...
matrix = as.matrix(table(data_prefs$country, data_prefs$sexual_orientation), header=TRUE, row.names=1)
table(matrix < 5) # absolute number of cells containing less than 5 counts
nrow(matrix) * 2 # number of cells

table(matrix<5)/(nrow(matrix)*2) # 53% of cells have counts less than 5 

# running Fisher's exact test
fisher_test_country = fisher.test(matrix)
...
# matrix = as.matrix(table(data_prefs$country, data_prefs$sexual_orientation), header=TRUE,
#                      row.names=1)
# 
# cramerV(matrix, ci = T, conf = 0.95)
# ?cramerV
# 
# matrix2 = as.matrix(table(data_prefs$country, data_prefs$sexual_orientation))
# 
# matrix
# 
# library(DescTools)
...
table(matrix_asex_v_umhetero_country<5)/(nrow(matrix_asex_v_umhetero_country)*2) # 64% of cells have counts less than 5 
...
# chi square test
chisq_country = chisq.test(asex_v_umhetero_prefs$sexual_orientation, asex_v_umhetero_prefs$country) 
chisq_country 
chisq_country$observed # observed counts
round(chisq_country$expected,2) # expected counts
...
table(matrix_hetero_v_umhetero_country<5)/(nrow(matrix_hetero_v_umhetero_country)*2) # 64 % of cells have counts less than 5 

# running Fisher's exact test
fisher_test_hetero_v_umhetero_country = fisher.test(matrix_hetero_v_umhetero_country)
fisher_test_country$p.value
