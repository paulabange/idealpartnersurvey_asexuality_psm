...
### Add variable indicating being excluded or included to each data set
```{r}
...
### Sexual orientation
```{r}
chisq_sexor = chisq.test(data_exclusion$exclusion, data_exclusion$sexual_orientation) 
chisq_sexor
...
Categories: Woman; Genderqueer/Nonbinary; Man; None of the above; Prefer not to say; Other
```{r}
chisq_sex = chisq.test(data_exclusion$exclusion, data_exclusion$sex) 
chisq_sex
chisq_sex$observed
round(chisq_sex$expected,2)
```
Not identifying as female was an exclusion criterion; therefore, included and excluded participants are certain to be different on this variable. 

### Relationship status
```{r}
chisq_relationship_status = chisq.test(data_exclusion$exclusion, data_exclusion$relationship_status) 
