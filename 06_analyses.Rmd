...
```{r}
ca =data_prefs %>% filter(sexual_orientation == "Straight/Heterosexual") %>% 
  select(pref_imp_ca) 
ca_asex = data_prefs %>% filter(sexual_orientation == "Asexual") %>% 
  select(pref_imp_ca) 

mean(ca$pref_imp_ca, na.rm = T)
mean(ca_asex$pref_imp_ca, na.rm = T)
mean(data_prefs$pref_imp_ca, na.rm = T)
