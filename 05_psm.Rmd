...
- *matching algorithm resulting in smds < 0.1 across the largest number of covariates will be chosen as algorithm for all analyses*
...
bal.tab(om, m.threshold = 0.1, un = T, binary = "std") # smd
bal.tab(om, v.threshold = 2, un = T) #vr (can only be calculated for continuous variables)
```
Based on SMDs, data is not balanced for 27 covariates. For continuous variables all variance ratios are close to 1.
...
bal.plot(om, var.name = "country", which = "both", grid = T) # not readable
...
v <- data.frame(old = c("distance", "language_chinese", "language_japanese", "language_german","language_english",
                        "language_italian", "language_danish", "language_spanish", "language_russian", 
                        "language_portuguese", "language_french", "age",
                        "relationship_status_Long-term", "relationship_status_No relationship", "relationship_status_Short-term",
                        "relationship_length"),
                new = c("Propensity scores", "Chinese", "Japanese", "German", "English", "Italian", "Danish", "Spanish", 
                        "Russian", "Portuguese", "French",
...
love.plot(bal.tab(om_without_country, m.threshold = 0.1, binardy = "std"),
          ...
          var.names = v) 
```
Optimal matching results in 27 covariates not being balanced after matching (based on SMDs). 

#### Matching with nearest-neighbor-algorithm without caliper
```{r}
...
#### Matching with nearest-neighbor-algorithm with caliper = 0.25*sd 
```{r}
...
love.plot(bal.tab(nnm_25_without_country, m.threshold = 0.1),
          ...
          abs = F) 
```
Unmatched asexual lies within the area of common support. 
...
love.plot(bal.tab(nnm_10_without_country, m.threshold = 0.1),
          ...
          abs = F) 
```
All unmatched asexuals lie within the area of common support. 

## Comparison of matching algorithms
### Smd and percent balance improvement
Comparing percent balance improvement after matching 
```{r}
...
table(comp2_smd)
```
Setting a caliper width did bring minor improvement in balance, mostly for some of the country variables. However, improvements were only minor but both algorithms did lead to asexual participants being discarded. Since without caliper adjustment balance criteria were already sufficed and no asexual participant was lost in the process, using nearest-neighbor-matching without a caliper is chosen. 


### Visualize
Variable country was left out for a clearer visualization
...
love.plot(sexual_orientation ~ language + age + 
          ...
          weights = list(w1 = get.w(om),
                         ...
                         w4 = get.w(nnm_10)),
          thresholds = c(m = .1),
          sample.names = c("Before Matching", "Optimal Matching", "Nearest-Neighbor-Matching",
                           "Nearest-Neighbor-Matching with caliper = 0.25", "Nearest-Neighbor-Matching with caliper = 0.10"),
          var.names = v,
          s.d.denom = "treated")
```
...
### Overview
```{r}
describeBy(nnm_data,nnm_data$sexual_orientation)
```


## 2. PSM for self-ratings
### Check for missings in covariates to be matched
```{r}
# MatchIt does not allow missing values
...
### Overview 
```{r}
describeBy(nnm_self_data, nnm_self_data$sexual_orientation) 
```


## 3. PSM for relationship options
### Check for missings in covariates to be matched
```{r}
# MatchIt does not allow missing values
...
### Overview
```{r}
describeBy(nnm_rel_data, nnm_rel_data$sexual_orientation) 
```
...
## Save unmatched heterosexuals for each psm sample as data files
```{r, eval = FALSE}
